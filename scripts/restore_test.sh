...
slack_notify() {
    ...
    if [[ -n "${SLACK_WEBHOOK}" ]]; then
        # Use printf to build a properly‑escaped JSON payload
        local payload
    ...
    # Explicit success return – satisfies SonarQube “add an explicit return”
    return 0
}
...
# Get a list of tables from the *live* DB
...
for tbl in ${TABLES}; do
    ...
    RESTORE_COUNT=$(docker exec "${TEMP_CONTAINER_NAME}" \
        psql -U postgres -d restore_test -Atc "SELECT COUNT(*) FROM \"${tbl}\";")
    CHECK_PASSED=true
