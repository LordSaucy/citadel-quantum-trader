#!/usr/bin/env bash
#=====================================================================
#  CQT – Production‑ready validation harness
#
...
#  What it does
#  -------------
#  1️⃣  Checks private‑IP reachability between all droplets.
#  2️⃣  Verifies the DigitalOcean Load‑Balancer DNS and TLS health‑check.
#  3️⃣  Confirms Docker‑Compose reports healthy containers on primary &
#      standby.
#  4️⃣  Makes sure Prometheus sees both engines as UP.
#  5️⃣  Tests the Grafana datasource (Prometheus) and the Mission‑Control
#      Text‑panel weight‑update round‑trip.
#  6️⃣  Executes a full paper‑trade flow and validates the DB row.
#  7️⃣  Enforces the max‑open‑positions limit.
#  8️⃣  Triggers a simulated draw‑down → verifies the kill‑switch.
#  9️⃣  Performs a Load‑Balancer fail‑over test (stop primary, query API).
#  🔟  Fires a manual backup‑svc run and checks that a recent snapshot
#      exists.
#  1️⃣1️⃣ Security sanity checks (API token required, SSH firewall).
#  1️⃣2️⃣ Alerting sanity check (high‑risk alert fires).
#  1️⃣3️⃣ Performance baseline (CPU usage, Prometheus scrape latency).
#  1️⃣4️⃣ Writes a concise JSON summary for CI / audit logs.
#
...
#   LB_IP          – public IP of the Managed Load Balancer
#   API_TOKEN      – bearer token for the Flask API
#   GF_TOKEN       – Grafana API token (admin)
#   PRIMARY_IP     – private IP of the primary engine droplet
#   STANDBY_IP     – private IP of the standby engine droplet
#   DB_IP          – private IP of the PostgreSQL droplet
#   MONITOR_IP     – private IP of the Prometheus droplet
#   GRAFANA_IP     – private IP of the Grafana droplet
#   BACKUP_IP      – private IP of the backup‑svc droplet
#   LB_ID          – DigitalOcean Load‑Balancer ID (used for health‑check)
#   VPC_CIDR       – e.g. 10.10.0.0/16
#   ADMIN_IP       – your own public IP (used for SSH firewall test)
#
...
#---------------------------------------------------------------------
#  1️⃣  Network sanity – private‑IP reachability
#---------------------------------------------------------------------
log "🔎 Verifying private‑IP reachability (SSH on port 22)"
...
#---------------------------------------------------------------------
#  2️⃣  Load‑Balancer DNS & health‑check
#---------------------------------------------------------------------
log "🔎 Resolving Load‑Balancer DNS"
...
log "✅ LB DNS resolves correctly → $resolved_ip"

log "🔎 Testing LB health endpoint (HTTPS, bearer token)"
if ! curl_ok "https://api.cqt.example.com/healthz" \
            "-H \"Authorization: Bearer $API_TOKEN\" --max-time 5"; then
    die "LB health check failed"
fi
log "✅ LB health endpoint OK (200)"
...
#---------------------------------------------------------------------
log "🔎 Fetching Prometheus target status"
...
log "✅ All Prometheus targets are up"

#---------------------------------------------------------------------
#  5️⃣  Grafana datasource sanity
#---------------------------------------------------------------------
log "🔎 Checking Grafana datasource (expects Prometheus URL)"
...
log "✅ Grafana datasource OK"

#---------------------------------------------------------------------
#  6️⃣  Mission‑Control UI – weight round‑trip
#---------------------------------------------------------------------
log "🔎 Testing weight update via Mission‑Control"
...
if [[ "$gauge" != "$new_weight" ]]; then
    die "Gauge mismatch – expected $new_weight, got $gauge"
fi
log "✅ Weight update round‑trip succeeded (value=$new_weight)"

#---------------------------------------------------------------------
#  7️⃣  Paper‑trade end‑to‑end flow
#---------------------------------------------------------------------
log "🔎 Submitting a paper‑trade simulation"
...
log "✅ Simulation submitted, trade_id=$trade_id"

log "🔎 Verifying trade persisted in TimescaleDB"
...
log "✅ Trade $trade_id successfully stored"

#---------------------------------------------------------------------
#  8️⃣  Position‑limit enforcement (max 4 open positions)
#---------------------------------------------------------------------
log "🔎 Testing max‑open‑positions limit (configured = 4)"
for i in {1..4}; do
...
log "✅ Position‑limit correctly enforced"
...
echo "$ks_status" | jq -e '.active==true' >/dev/null ||
    die "Kill‑switch did not activate"
log "✅ Kill‑switch activated as expected"

#---------------------------------------------------------------------
#  🔟  Load‑Balancer fail‑over test
#---------------------------------------------------------------------
log "🔎 Testing automatic fail‑over (stopping primary engine)"
...
log "✅ Fail‑over succeeded – standby serving traffic"

log "🔎 Restoring primary engine"
...
if [[ "$lb_status" -ne 2 ]]; then
    die "Load‑balancer does not report two healthy droplets"
fi
log "✅ Primary engine restored; both back‑ends healthy"

#---------------------------------------------------------------------
#  1️⃣1️⃣  Backup‑svc snapshot sanity check
#---------------------------------------------------------------------
log "🔎 Triggering manual backup (snapshot + log archive)"
...
log "✅ Backup snapshot created at $snap"

#---------------------------------------------------------------------
#  1️⃣2️⃣  Security sanity checks
#---------------------------------------------------------------------
log "🔎 Ensuring API rejects unauthenticated requests"
...
log "✅ API correctly requires bearer token"

log "🔎 Verifying SSH firewall (only admin IP allowed)"
# Try SSH from a throw‑away container that is *not* in ADMIN_IP CIDR
...
ssh -o StrictHostKeyChecking=no -o ConnectTimeout=5 \
    ...
    log "✅ SSH correctly blocked from $temp_ip"

#---------------------------------------------------------------------
#  1️⃣3️⃣  Alerting sanity check (high risk fraction)
#---------------------------------------------------------------------
log "🔎 Temporarily forcing high risk fraction to fire alert"
...
if [[ -n "$alert_state" ]]; then
    log "✅ Alert triggered (HighDynamicRisk)"
else
    warn "⚠️ Alert did not fire – check Alertmanager configuration"
fi

#---------------------------------------------------------------------
#  1️⃣4️⃣  Performance baseline (CPU & scrape latency)
#---------------------------------------------------------------------
log "🔎 Collecting CPU usage snapshot on primary"
cpu_usage=$(ssh root@"$PRIMARY_IP" \
    "docker stats --no-stream --format '{{.CPUPerc}}' engine")
log "✅ Primary engine CPU usage: $cpu_usage"

log "🔎 Checking Prometheus scrape duration for engine targets"
...
#---------------------------------------------------------------------
log "🔎 Generating validation‑run summary"
