...
Loads MT5‑exported CSVs (M15, H1, H4, D1, W1) and the alignment
matrix generated by the MT5 script.  Provides:

* Trend‑alignment scoring
* Identification of the best alignment periods
* Helpers for back‑testing / strategy optimisation
...
class MTFDataAnalyzer:
    ...
    * ``load_all_data()`` – pulls CSVs into pandas DataFrames.
    * ``should_trade_now(direction, min_alignment)`` – quick alignment check.
    * ``get_best_alignment_periods(min_score, min_duration_hours)`` – returns
      a list of high‑quality periods.
    * ``get_summary()`` – human‑readable text for logs / UI.
    * ``export_alignment_summary(path)`` – JSON dump for offline analysis.
    ...
    def __init__(self, data_prefix: str = "mtf_data", symbol: str = "EURUSD"):
        ...
        # DataFrames for each timeframe – will be filled by load_all_data()
        ...
        # Runtime‑mutable configuration (starts with defaults)
        ...
        logger.info("📊 MTF Data Analyzer initialised")
    ...
    def _load_timeframe(self, tf: str) -> bool:
        ...
        if not csv_path:
            logger.warning(f"⚠️ {tf} data not found: {filename}")
            return False
        ...
        try:
            ...
            logger.error(f"❌ Error loading {tf}: {exc}")
            return False
    ...
    def _load_alignment(self) -> bool:
        ...
        csv_path = self._find_csv_file(filename)
        if not csv_path:
            logger.warning("⚠️ Alignment CSV not found")
            return False
        ...
        try:
            ...
            df.set_index('datetime', inplace=True)
            self.alignment = df
            logger.info(f"✅ Alignment data loaded ({len(df)} rows)")
            return True
        except Exception as exc:
    ...
    def load_all_data(self) -> bool:
        ...
        if loaded:
            logger.info(f"✅ Successfully loaded {loaded} timeframe(s)")
            return True
        else:
    ...
    def should_trade_now(self, direction: str,
        ...
        if primary != direction:
            return False, f"Primary trend {primary} ≠ {direction}", score
        if score < threshold:
    ...
    def get_timeframe_data(self,
        ...
        if df is None:
            logger.warning(f"⚠️ {timeframe} data not loaded")
            return None
    ...
    def export_alignment_summary(self,
        ...
        try:
            ...
            logger.info(f"✅ Alignment summary exported to {output_path}")
            return True
        except Exception as exc:
            logger.error(f"❌ Failed to export summary: {exc}")
            return False

    def get_summary(self) -> str:
        """Human‑readable multi‑line summary (good for logs or UI)."""
        lines = ["═" * 70,
                 "       MULTI‑TIMEFRAME DATA ANALYZER SUMMARY",
                 "═" * 70,
        ...
        # Timeframe overview
        lines.append("📈 Loaded Timeframes:")
        for tf in ['M15', 'H1', 'H4', 'D1', 'W1']:
            df = getattr(self, tf.lower())
            if df is not None:
                lines.append(f"  {tf}: {len(df):,} rows ({df.index[0]} → {df.index[-1]})")
            else:
        ...
        if self.alignment is not None:
            ...
            # Trend distribution
            lines.append("")
            lines.append("📊 Primary Trend Distribution:")
            ...
            lines.append("")
            lines.append("⚠️ No alignment data loaded")
    ...
    def set_config(self, key: str, value: float) -> None:
        ...
        try:
            ...
            logger.info(f"Config updated – {key} = {self.config[key]}")
        except Exception as exc:   # pragma: no cover
    ...
    def _start_api(self, host: str = "0.0.0.0", port: int = 5006) -> None:
        ...
        * GET  /summary          → multi‑line human readable summary
        * GET  /config           → JSON of all tunable parameters
        ...
        * GET  /healthz          → simple health check
        ...
        def _run():
            # Flask's built‑in server is sufficient for internal use.
...
def _demo():
    ...
    if not mtf_analyzer.load_all_data():
        logger.error("Failed to load any data – aborting demo")
        return
    ...
    print(f"\nShould we BUY now? {'YES' if ok else 'NO'} – {reason} (score {score:.1f}%)")
    ...
    try:
        ...
        logger.info("Demo interrupted – exiting")
        mtf_analyzer.stop()
