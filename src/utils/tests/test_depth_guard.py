#!/usr/bin/env python3
"""
tests/test_depth_guard.py (or src/utils/tests/test_depth_guard.py)

Tests for depth guard functionality that prevents over-exposure to
illiquid price levels in the order book.
...
def test_long_trade_passes_when_enough_ask_volume(dummy_dom):
    """Test that long trade is approved when sufficient ask volume exists."""
    entry_price = 1.0800
...
def test_long_trade_fails_when_not_enough_ask_volume(dummy_dom):
    """Test that long trade is rejected when ask volume is insufficient."""
...
def test_short_trade_uses_bid_volume(dummy_dom):
    """Test that short trades check bid volume instead of ask."""
    entry_price = 1.0800
...
def test_price_to_lots_basic():
    """
    Test basic price-to-lots conversion.
    ...
    # Old: assert pytest.approx(lots, rel=1e-2) == 0.054
    # New: assert lots == pytest.approx(0.054, rel=1e-2)
    assert lots == pytest.approx(0.054, rel=1e-2)
